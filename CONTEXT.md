...
nobody's identity decides which (ADR-0022). Each Runtime brings its own pinned
...
every spawn so that no Session runs on an effort nobody chose (ADR-0023).
...
the other's Sessions untouched (ADR-0022).
...
Task that hits its window fails with the reason instead, and ADR-0023 is where
...
on a person (ADR-0022). Distinct from queued — the Task Queue decides what may
