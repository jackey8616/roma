/**
 * Which Tasks obtained a Cloud Token, until their Audit Record has said so.
 *
 ...
 * itself is invisible here, which is the same honesty the `Requested-by:`
 * trailer carries.
 */
export class CloudReachUse {
  readonly #tasks = new Set<string>()

  /**
   * A Cloud Token was handed to whatever this Task's Session was running.
   *
  ...
  /**
   * Whether this Task used the Cloud Reach, asked once and then forgotten.
   *
   ...
   * credential that paid — and both say the same thing about the cloud, so the
