 ...
 *
 * A missing record is the ordinary case — almost every Conversation has never
 * used `/clear` and almost every Session runs on the Pinned Model — and every
 * *other* failure describes a record that may well exist and cannot be read.
 * Answering "there is none" to those is how a Chosen Model disappears with
 * nobody told.
 *
 * Its own function for one caller, which is worth saying out loud: it was three
 * before the read moved here, and what it is now is a name for the distinction
 * rather than a saving. The name is the point — `catch { return null }` is the
 * bug this is spelled out to prevent.
 */
