...
| `ROMA_CAVEMAN` | Sets the **Pinned Caveman** — how short roma asks the model to be, on every Session nobody has moved. **Unset by default, and unset means untouched**: nothing is appended and a Session's spawn arguments are byte for byte what they were before this existed. Takes any level a Caller will be able to choose (`off`, `lite`, `full`, `ultra`, `wenyan-full`) and also `wenyan-lite` and `wenyan-ultra`, which are off the Caveman Menu and reachable only from here — wenyan's own rules claim their saving in *characters* where a Shared Window is spent in tokens, and roma will not draw a button it cannot account for. `off` is a level rather than the absence of one, and it means no text at all. What gets appended is roma's own text, derived from the third-party **caveman** skill (`github.com/JuliusBrussee/caveman`, MIT) at a pinned commit, with the lines rewritten that describe machinery roma deliberately does not install — no hooks, no plugin, nothing of caveman's running in the container. **caveman claims 65% off prose output. That is caveman's measurement of caveman, on prompts nobody here has seen, and roma claims nothing** until its own Audit Records can answer it (ADR-0030). **Validated at startup**, and here that is the only line of defence there is: unlike a mistyped `--effort`, which at least reaches Claude Code and gets a warning, a mistyped Caveman reaches nothing — the ruleset is assembled by roma and handed over already rendered, so no Runtime ever sees the level. |
