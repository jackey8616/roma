...
what answers to `gh` is roma's Credential Shim, which mints a token for the one invocation
it wraps.
...
| `ROMA_GITHUB_PRIVATE_KEY_FILE` | **Required.** A path to the App's private key, PEM. A path rather than the key inline, following `GOOGLE_APPLICATION_CREDENTIALS`: multi-line secrets belong in mounts. roma holds this key and nothing in the agent's reach ever sees it. |
...
| `ROMA_GH_BIN` | Where the real `gh` is, for the Shim in front of it. `/usr/local/lib/roma/gh` by default, which is where the image puts it. |
